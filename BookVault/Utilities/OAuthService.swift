...
public extension URLRequest
{
    ...
    ///   - consumerCredentials: consumer credentials
    ///   - userCredentials: user credentials (nil if this is a request without user association)
    mutating func oAuthSign(method: String, urlFormPaOAuthServicerameters paras: [String: String],
                            consumerCredentials cc: OAuthService.Credentials, userCredentials uc: OAuthService.Credentials? = nil)
    {
        self.httpMethod = method.uppercased()

        let body = OAuthService.httpBody(forFormParameters: paras)
        ...
        let sig = OAuthService.calculateSignature(url: self.url!, method: self.httpMethod!,
